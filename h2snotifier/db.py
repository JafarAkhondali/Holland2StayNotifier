...
house_columns = ['url_key', 'area', 'city', 'price_exc', 'price_inc',
                 'available_from', 'max_register', 'contract_type',
                 'min_stay', 'rooms']
# Non-mass assignables: 'created_at', 'occupied_at'

# Configure logging
logging.basicConfig(filename='house_sync.log', level=logging.INFO,
...
def sync_houses(city_id, houses):
    ...
    try:
        ...
        to_be_inserted = [tuple(house[column] for column in house_columns) for house in houses if
                          house['url_key'] not in existing_houses]
